...
curl -X POST -H "Content-type: application/json" --data '{"attachments": [{"mrkdwn_in": ["text","title"], "color": "#000000", "author_name": "'"$GITHUB_ACTOR"'","author_link": "https://github.com/'"$GITHUB_ACTOR"'","author_icon": "'"$INPUT_AUTHOR_AVATAR"'", "text": "*<'"$INPUT_PR_LINK/$INPUT_COMMIT_SHA"'|1 new commit> pushed to `'"$GITHUB_HEAD_REF"'`*\n<'"$INPUT_PR_LINK/$INPUT_COMMIT_SHA"'|`'"${INPUT_COMMIT_SHA::8}"'`> - '"$INPUT_MESSAGE"'"} ]}' $INPUT_SLACK_WEBHOOK
